  ...
  class TransactionJob < ::ActiveJob::Base
    ...
    def valid_job?(transaction)
      ...
      if transaction.transaction_id.blank?
        Accounting.log 'Transaction', 'Job', { info: "Transaction: #{transaction.id} - no transaction id, processing..." }
        true
      else
        Accounting.log 'Transaction', 'Job', { info: "Transaction: #{transaction.id} - already has transaction id, skipping..." }
        false
      end
    end
